...
## 📂 Cấu Trúc Dự Án (Clean Architecture & Directory Structure)

Dự án được thiết kế theo mô hình **Clean Architecture**, tách biệt hoàn toàn giữa các tầng giao diện, nghiệp vụ cốt lõi và truy cập cơ sở dữ liệu:

```text
DormitoryManagementSystem/
├── src/                                      # Thư mục mã nguồn chính của dự án
│   ├── DormitoryManagement/                  # [Presentation Layer] Dự án chính ASP.NET Core MVC & Web API
│   │   ├── Controllers/                      - Tiếp nhận HTTP Request, gọi Services, trả về View hoặc JSON
│   │   ├── Views/                            - Giao diện người dùng Razor View (.cshtml)
│   │   ├── wwwroot/                          - Thư mục tài nguyên tĩnh (custom CSS, JS, thư viện Bootstrap)
│   │   ├── Program.cs                        - Điểm khởi chạy ứng dụng, đăng ký cấu hình JWT, định nghĩa Middleware
│   │   └── appsettings.json                  - File cấu hình hệ thống (SQL ConnectionString, JwtSettings, Mail)
│   │
│   ├── DormitoryManagement.Application/      # [Application Layer] Chứa logic nghiệp vụ cốt lõi
│   │   ├── Dtos/                             - Lớp truyền tải dữ liệu Request/Response
│   │   ├── Mappings/                         - Cấu hình ánh xạ tự động AutoMapper Profile
│   │   ├── Services/                         - Lớp xử lý nghiệp vụ (Hợp đồng, Phòng ở, Thu tiền điện nước)
│   │   ├── Validators/                       - Bộ lọc kiểm tra tính hợp lệ dữ liệu (FluentValidation)
│   │   └── DependencyInjection.cs            - Đăng ký DI cho các dịch vụ tầng Application
│   │
│   ├── DormitoryManagement.Infrastructure/   # [Infrastructure Layer] Tầng hạ tầng và giao tiếp CSDL
│   │   ├── Data/                             - EF Core DbContext (ApplicationDbContext), DbSeeder (Seed Data) và Migrations
│   │   ├── Repositories/                     - Triển khai chi tiết các câu lệnh truy vấn SQL qua EF Core
│   │   ├── Services/                         - Lớp gửi EmailService, v.v.
│   │   └── DependencyInjection.cs            - Đăng ký DI cho DbContext, UnitOfWork và các Repositories
│   │
│   ├── DormitoryManagement.Domain/           # [Domain Layer] Tầng chứa các thực thể cốt lõi (Hoàn toàn độc lập)
│   │   ├── Entities/                         - Model Database đại diện cho các bảng (User, Room, Bed, Contract...)
│   │   ├── Enums/                            - Chứa phân loại vai trò, trạng thái hóa đơn, hợp đồng
│   │   └── Interfaces/                       - Khai báo giao diện Repositories và IUnitOfWork
│   │
│   └── DormitoryManagement.Common/           # [Common Layer] Tầng chứa các tiện ích dùng chung
│       ├── Constants/                        - Định nghĩa hằng số hệ thống, thông báo lỗi chuẩn hóa
│       └── Helpers/                          - Các lớp trợ giúp mã hóa, băm mật khẩu, định dạng dữ liệu
```

> [!NOTE]
> **Tài liệu hướng dẫn lập trình Local:**
> Dự án chứa thư mục `docs_flow/` (không đẩy lên Git) dành cho các tài liệu kỹ thuật chuyên sâu. Bạn có thể mở tệp local `docs_flow/index.md` bằng IDE để tra cứu sơ đồ tuần tự backend, luồng xác thực JWT, và hướng dẫn chi tiết về quy trình đóng góp code.
...
Hệ thống tài liệu hướng dẫn đặc tả kiến trúc, luồng xử lý và dữ liệu của dự án nằm trong thư mục `src/DormitoryManagement/Docs/` (được đẩy lên Github):
...
> [!NOTE]
> **Tài liệu hướng dẫn lập trình Local:**
> Dự án có chứa thêm một cổng thông tin tài liệu kỹ thuật dành cho nhà phát triển để xem dưới máy local (nằm trong thư mục `docs_flow/` - được bỏ qua không đẩy lên Git). Bạn có thể mở tệp local `docs_flow/index.md` bằng trình xem Markdown của IDE để tra cứu sơ đồ tuần tự backend, luồng xác thực JWT Bearer Cookie chi tiết, cách viết validation, và cách đóng góp code ở từng tầng kiến trúc.
